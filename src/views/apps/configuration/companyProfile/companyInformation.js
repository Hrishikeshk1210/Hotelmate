...
let rateCodes = [
  fetch("http://192.168.1.33:14700/getAccountsRateCode?hotelID=1")
...
let accountManager = [
  fetch("http://192.168.1.33:14700/getAccountUser?hotelID=1")
    .then((result) => result.json())
    .then((resp) => {
      // console.log(resp['data'])
...
const Floor = ({ stepper, data1 }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (data.accountType !== null && data.accountName !== null) {
      ...
      console.log("hi");
      let res = fetch("http://192.168.1.33:14700/addAccounts", {
        method: "POST",
        ...
        .then((result) => result.json())
        .then((resp) => {
          localStorage.setItem("companyID", resp["data"]);
