...
const ValidationThirdPartyComponents = ({ floorid,roomAssign }) => {
  ...
  const onSubmit = (data) => {
    ...
    if (data.floorID !== null && data.blockID !== null) {
      ...
      let res = fetch("http://192.168.1.33:14700/updateRoomStatus?id=" + localStorage.getItem('id'), {
        method: "PUT",
        ...
        console.log(res);
        // if (res['status'] == 200) {
        //   fetch('http://192.168.1.33:14700/addOutOfOrderOrService')
        //     .then(result => result.json())
        //     .then(rowData => {
        ...
        if (data.fromDate !== null && data.toDate !== null) {
          ...
          console.log(createmarketGroup)
          let res = fetch("http://192.168.1.33:14700/addOutOfOrderOrService", {
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            body: createmarketGroup
          }).then((res) => {
  ...
  function Alert() {
    ...
    setTimeout(() => { navigate('/dashboard/roomManagement/availability'); }, 200)
  }
